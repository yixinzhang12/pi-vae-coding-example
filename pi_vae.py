...
def first_nflow_layer(z_input, dim_x, min_gen_nodes=30):
    ...
    dim_z = z_input.shape.as_list()[-1];##number of features in last latend dimension
    n_nodes = [gen_nodes, gen_nodes, dim_x-dim_z]; ## number of nodes in each layer
    act_func = ['relu', 'relu', 'linear']; 
    n_layers = len(n_nodes);
    output = z_input;
    
    for ii in range(n_layers): ## iterates over each layer
        output = layers.Dense(n_nodes[ii], activation=act_func[ii])(output); ##output has dim_x-dim_z elements
    
    output = layers.concatenate([z_input, output], axis=-1); ##mix original with more complex data
    return output
...
def affine_coupling_layer(layer_input, min_gen_nodes=30, dd=None):
    ...
    """
    DD = layer_input.shape.as_list()[-1]; ## DD needs to be an even number ##number of features in the layer
    if dd is None:
        dd = (DD//2);
    
    ## define some lambda functions
    clamp_func = Lambda(lambda x: 0.1*tf.tanh(x)); ##scales tanh, keep values small
    trans_func = Lambda(lambda x: x[0]*tf.exp(x[1]) + x[2]); ##exponential transformation
    sum_func = Lambda(lambda x: K.sum(-x, axis=-1, keepdims=True));##sum of negative of last axis input
    
    ## compute output for s and t functions, both from dd to DD-dd
    x_input1 = Lambda(slice_func, arguments={'start':0,'size':dd})(layer_input); #slice from 0 to dd
    x_input2 = Lambda(slice_func, arguments={'start':dd,'size':DD-dd})(layer_input); ## dd to DD-dd
    st_output = x_input1;
...
def affine_coupling_block(x_output, min_gen_nodes=30, dd=None):
    ...
    """
    for _ in range(2):##chains two affine coupling layers together
...
def decode_nflow_func(z_input, n_blk, dim_x, mdl, min_gen_nodes=30, dd=None):
    ...
    for ii in range(n_blk):
        np.random.seed(ii);
        permute_ind.append(tf.convert_to_tensor(np.random.permutation(dim_x))); ##generate random permutation of itegers up to dim_x
    
    ## Get output through first_nflow_layer 
    output = first_nflow_layer(z_input, dim_x, min_gen_nodes);  ##apply fist nflow
    
    ## First permute the input before passing it to each GIN block (affine_coupling_block). Repeat this procedure n_blk times.
    for ii in range(n_blk): ## apply affine coupling
        output = Lambda(perm_func, arguments={'ind':permute_ind[ii]})(output); ##permutation based on id, ensure rich transformation
    ...
    if mdl == 'poisson': ##transformation of poisson (count data)
...
def z_prior_nn(u_input, dim_z): ##input, output dim ## continuous prior
    ...
    n_nodes = [n_hidden_nodes_in_prior, n_hidden_nodes_in_prior, 2*dim_z]; ## 2*dim_z if gaussian prior for mean and log var
    act_func = ['tanh', 'tanh', 'linear'];
    n_layers = len(n_nodes);
    output = u_input;
    ...
    lam_mean = Lambda(slice_func, arguments={'start':0,'size':dim_z})(output); ##slice first half as mean of gaussian prior
    lam_log_var = Lambda(slice_func, arguments={'start':dim_z,'size':dim_z})(output); ##second half as log var
...
def z_prior_disc(u_input, dim_z, num_u):
    ...
    # Returns
        mean and log of variance of prior p(z|u) (tensors)
    """
    lam_mean = squeeze_func(layers.Embedding(num_u, dim_z, input_length=1)(u_input)) #the embedding layers are used to map each discrete label in u_input to vectors representing the mean and log variance of the Gaussian prior
    lam_log_var = squeeze_func(layers.Embedding(num_u, dim_z, input_length=1)(u_input))
    return lam_mean, lam_log_var

def sampling(args): ## reparameterization, since z cannot be random for backpropagation
...
def compute_posterior(args):
    ...
    """
    z_mean, z_log_var, lam_mean, lam_log_var = args; ##mean of latent, log var of latent, mean of prior, log var of prior distr
    # q(z) = q(z|x)p(z|u) = N((mu1*var2+mu2*var1)/(var1+var2), var1*var2/(var1+var2));
    post_mean = (z_mean/(1+K.exp(z_log_var-lam_log_var))) + (lam_mean/(1+K.exp(lam_log_var-z_log_var))); ##mean of posterior gaussian distribution
    post_log_var = z_log_var + lam_log_var - K.log(K.exp(z_log_var) + K.exp(lam_log_var)); ##log var of post
...
def realnvp_layer(x_input): ##real-valued no-volume preserving model, a type of nflow
    DD = x_input.shape.as_list()[-1]; ## DD needs to be an even number , x input dimension
    dd = (DD//2);
